    ...
    function save(): void {
        ...
        CUtils.saveItem(screencopy, tmpfile, Qt.rect(Math.ceil(rsx), Math.ceil(rsy), Math.floor(sw), Math.floor(sh)), path => {
            if (root.loader.clipboardOnly) {
                ...
                Quickshell.execDetached(["sh", "-c", `notify-send -a 'caelestia-cli' -i '${path}' 'Screenshot taken' 'Screenshot copied to clipboard' -A 'open=Open' --wait | grep -q '^open$' && imv '${path}'`]);
            } else {
